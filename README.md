...
```
~/.pi/agent/extensions/twitter-broadcast-chat/
```

Or clone and point to it in `~/.pi/agent/settings.json`:

```json
{
  "extensions": ["/path/to/twitter-broadcast-chat"]
}
```
...
4. Polls `/chatapi/v1/history` on the chat replay endpoint
5. Parses `kind === 1` messages from the nested JSON payload
...
| File | Purpose |
|------|---------|
| `index.ts` | Pi extension entry point, commands, widget, polling |
| `twitter-chat-api.ts` | X broadcast bootstrap and chat history fetching |
